...
var gServiceAppId = "vd84JCg9vN.QRDemo";
...
var init = function () {
    ...
    if (localStorage.history == undefined)
    {
      ...
      localStorage.setItem("histnames", gHistNames);
      localStorage.setItem("histstops", gHistStops);
      localStorage.setItem("histlen", 10);
    } else {
    	var storedData = localStorage.getItem("histnames");
    	if (storedData) {
    	    gHistNames = JSON.parse(storedData);
    	} 
    	var storedData = localStorage.getItem("histstops");
    	if (storedData) {
    	    gHistStops = JSON.parse(storedData);
    	} 
    }
    histlen = localStorage.getItem("histlen");
...
function showPosition(position)
{
	...
	$.get( url, { lat: position.coords.latitude, lon: position.coords.longitude } ).done(function( data ) {
		...
		{
			gid = info[i].uniq_group_id;
			if (gids.indexOf(gid) == -1)
			{
				gids.push(gid);
				groups.push([]);
			}
			groups[gids.indexOf(gid)].push(i);
		}
		console.log(info.length + " > " + gids.length);
		for (var i = 0; i < Math.min(groups.length, 6); i++)
		{
			//item = "<li class=\"ui-li-has-multiline\">";
			stoplist = [];
			for (j = 0; j < groups[i].length; j++)
				stoplist.push(info[groups[i][j]]);
			si = info[groups[i][0]];
			type = "<img style=\"margin-top: 8Px;\" src=\"img/icon_"+si.transport_type + "_white32.png\" />";
			dest = "";
			if (si.routeinfo != undefined)
				{
					dest = ", до " + si.routeinfo[0].poiFinish.replace(/"/g, "");
				}
			//~ <span class=\"ui-li-text-sub\">"+info[i].timeToArrive+" мин</span> //~ add for some routes coming
			stopname = si.stop_name.replace(/"/g, '');
			item = "<li class=\"ui-li-has-multiline\"><a href=\"#two\" class=\"stop\" onclick=\"go_stop("+si.stop_code+', \''+stopname+'\');">'+type+" "+stopname+"<span class=\"ui-li-text-sub\">"+(Math.round(info[i].distance * 100) * 10)+"м"+dest+"</span></a></li>";
			//$( "#stoplist").listview( 'addItem', "<div data-role=\"collapsible\" data-inset=\"false\"><li class=\"ui-li-dialogue\"><a href=\"#two\" class=\"stop\" onclick=\"stop_id="+si.stop_code+'; stop_name=\''+si.stop_name+'\'">'+type+" "+si.stop_name+"</a></li><li class=\"ui-li-dialogue\">Uncompressed!</li></div>", $("#buslist li").length );
           	$( "#stoplist").listview( 'addItem', item, $("#stoplist li").length );
		}
		if (info.length < 1)
...
function startMessagePort() {
	try {
		...
		alert("Could not launch interaction!");
	}
}
